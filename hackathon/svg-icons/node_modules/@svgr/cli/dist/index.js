  ...
  async function handle(filename, root) {
    ...
    if (stats.isDirectory()) {
      ...
      if (transformed.length) {
        ...
        const dest2 = path__namespace.resolve(outDir, path__namespace.relative(root, dirname));
        const resolvedConfig = core.loadConfig.sync(__spreadValues$1({ configFile }, opts), { filePath: dest2 });
...
const parseConfig = (name) => (arg) => {
  try {
    ...
    exitError(`"${name}" is not valid, please specify a valid file or use a inline JSON.`);
  }
};
...
commander.program.version(version).usage("[options] <file|directory>").option("--config-file <file>", "specify the path of the svgr config").option("--no-runtime-config", 'disable runtime config (".svgrrc", ".svgo.yml", ".prettierrc")').option("-d, --out-dir <dirname>", "output files into a directory").option("--ignore-existing", "ignore existing files when used with --out-dir").option("--ext <ext>", 'specify a custom file extension (default: "js")').option("--filename-case <case>", 'specify filename case ("pascal", "kebab", "camel") (default: "pascal")').option("--icon [size]", 'specify width and height (default to "1em" or 24dp (native))', parseIconSize).option("--jsx-runtime <runtime>", 'specify JSX runtime ("automatic", "classic", "classic-preact") (default: "classic")').option("--typescript", "transform svg into typescript").option("--native", "add react-native support with react-native-svg").option("--memo", "add React.memo into the result component").option("--ref", "forward ref to SVG root element").option("--no-dimensions", "remove width and height from root SVG tag").option("--expand-props [position]", 'disable props expanding ("start", "end", "none") (default: "end")', parseExpandProps).option("--svg-props <property=value>", "add props to the svg element", parseObjectList).option("--replace-attr-values <old=new>", "replace an attribute value", parseObjectList).option("--template <file>", "specify a custom template to use", parseTemplate("--template")).option("--index-template <file>", "specify a custom index.js template to use", parseTemplate("--index-template")).option("--no-index", "disable index file generation").option("--title-prop", "create a title element linked with props").option("--prettier-config <fileOrJson>", "Prettier config", parseConfig("--prettier-config")).option("--no-prettier", "disable Prettier").option("--svgo-config <fileOrJson>", "SVGO config", parseConfig("--svgo-config")).option("--no-svgo", "disable SVGO").option("--silent", "suppress output").option("--stdin", "force reading input from stdin").option("--stdin-filepath", "path to the file to pretend that stdin comes from");
